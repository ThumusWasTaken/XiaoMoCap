...
void handleData() {
  ...
  String csvData = "timestamp_iso";
  if (acc_active == true){csvData += ",ax,ay,az"; }
  if (gyr_active == true){csvData += ",gx,gy,gz"; }
  if (mag_active == true){csvData += ",mx,my,mz"; }
  csvData += "\n";
...
//   String csvHeader = "timestamp_iso";
//   if (acc_active) csvHeader += ",ax,ay,az";
//   if (gyr_active) csvHeader += ",gx,gy,gz";
//   if (mag_active) csvHeader += ",mx,my,mz";
//   csvHeader += "\n";
...
//       pageData += c;
//     }
//     server.sendContent(pageData);  // Send this chunk
//     delay(1000);  // Let WiFi breathe
//   }

//   server.sendContent("");  // End of stream
//   Serial.println("✔ CSV sent in chunks.");
// }
...
//   // Early DOWNLOAD check (within 2 seconds of boot)
...
//         Serial.println("💾 DOWNLOAD received during boot. Dumping logs...");

//         // SPI setup
//         SPI.begin();  // Critical for flash
//         pinMode(FLASH_CS, OUTPUT);
//         digitalWrite(FLASH_CS, HIGH);  // Deselect initially
//         delay(10);
...
//         uint8_t readBuf[2048];
//         uint16_t lastPage = 2048;
//         for (uint16_t page = 0; page < lastPage; page++) {
//           flash.readPage(page, readBuf, 2048);
//           // Check if this page is empty
//           bool isEmpty = true;
//           for (int i = 0; i < 2048; i++) {
//             if (readBuf[i] != 0xFF && readBuf[i] != 0x00) {
//               isEmpty = false;
//               break;
//             }
//           }
//           if (isEmpty) break;  // Stop reading if we hit empty
...
//         while (1);  // Halt – no further code runs
//       }
//     }
//   }
//   //if not USB download boot as normal
//   // I2C setup (Xiao ESP32-C3: SDA = 8, SCL = 9)
//   Wire.begin();
//   delay(100);
...
void loop() {
  ...
  while (millis() - startTime < duration_ms) {
    ...
    String line = String(timestamp) + str_acc + str_gyr + str_mag;
